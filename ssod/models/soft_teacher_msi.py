...
class SoftTeacherMSI(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(SoftTeacherMSI, self).__init__(
    ...
    def foward_unsup_train(self, teacher_data, student_data):
        ...
        feat, img_metas = self.teacher.extract_test_feat(
            teacher_data["img"][torch.Tensor(tidx).to(teacher_data["img"].device).long()],
            [teacher_data["img_metas"][idx] for idx in tidx],
        )
        with torch.no_grad():
            teacher_info = self.extract_teacher_info(feat, img_metas,
        ...
        hr_info = student_data
        lr_info = self.student.resize_info(hr_info, scale=0.5)

        hr_features = self.student.extract_feat(hr_info['img'])
        lr_features = self.student.extract_feat(lr_info['img'])
        fr_features = self.student.extract_fuse_feat(hr_features, lr_features)
        ...
        for res_key, info in multi_res_info.items():
            feat, img, img_metas = info['x'], info['img'], info['img_metas']
            student_info = self.extract_student_info(img, feat, img_metas)
            res_losses = self.compute_pseudo_label_loss(student_info, teacher_info)
            losses.update({k + f'_{res_key}': v for k, v in res_losses.items()})
        losses = self.student.reduce_losses(losses, len(multi_res_info))
        return losses
    ...
    def extract_student_info(self, img, feat, img_metas):
        student_info = {}
    ...
    def extract_teacher_info(self, feat, img_metas, proposals=None, **kwargs):
        teacher_info = {}
